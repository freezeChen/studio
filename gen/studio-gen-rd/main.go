...
func getOptions(opt *options, comment string) {
	...
	if regexp.MustCompile(`\s+//\s*rd:.+`).Match([]byte(comment)) {
		args := strings.Split(pkg.RegexpReplace(`//\s*rd:(?P<arg>.+)`, comment, "$arg"), " ")
